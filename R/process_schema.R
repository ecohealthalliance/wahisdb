...
process_schema <- function(schema_extract, wahis_tables, disease_key) {
  ...
  wahis_epi_events_schema <- tibble(field = colnames(wahis_tables$wahis_epi_events),
                                    field_type = map_chr(wahis_tables$wahis_epi_events, ~class(.)[1])) |>
    left_join(schema_extract) |>
    mutate(table = "wahis_epi_events") |>
  ...
  schema <- bind_rows(wahis_epi_events_schema, wahis_outbreaks_schema) |>
